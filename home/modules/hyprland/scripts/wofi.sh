...
while true; do
   ...
   SELECTED=$(ls "$WALLPAPER_DIR"/*.{png,jpg,jpeg,gif,webp} 2>/dev/null | xargs -n 1 basename | wofi --dmenu --prompt "Select a wallpaper:" -W 300 -H 300)
    ...
    swww img --transition-fps 120 --transition-type grow --transition-duration 0.5 --transition-bezier 1,0.9,1,0.7 --invert-y --transition-pos "$(hyprctl cursorpos | grep -E '^[0-9]' || echo "0,0")" "$WALLPAPER_DIR/$SELECTED" | cp  "$WALLPAPER_DIR/$SELECTED" $HOME/.cache/rofi-wallpapers/image.png
    else
